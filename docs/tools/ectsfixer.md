---
title: "📚 ECTS Fixer"
sidebar_label: "📚 ECTS Fixer"
sidebar_position: 6
description: Plugin die ects.ap.be wat pimpt — standaard academiejaar veranderen + 'open in nieuwe tab'-knoppen overal.
tags: [AP, Moodle, Chrome Extension]
---

<a className="button button--primary button--lg" href="https://github.com/timdams/Ectsfixer" target="_blank" rel="noopener noreferrer">⬇️ Download / Open project</a>

Kleine browserextensie (Chrome / Edge / Brave) die enkele ergernissen op
[ects.ap.be](https://ects.ap.be) wegwerkt. De extensie is enkel actief op die
domeinnaam.

## Wat doet ze?

- **Academiejaar automatisch instellen.** Bij het openen van de site wordt de
  academiejaar-dropdown standaard op het ingestelde jaar gezet (default
  `2025-2026`). Handig zolang de site bij elke load weer naar het lopende
  academiejaar springt.
- **"Open in nieuwe tab"-knop bij elk vak.** Op een programma-pagina (bv.
  `/ects/opleiding/<jaar>/<code>/<id>`) verschijnt naast elke vak-rij een klein
  knopje. Een klik opent dat vak in een nieuwe tab; de huidige tab blijft op
  het programma-overzicht staan.

Beide gedragingen zijn afzonderlijk uit te schakelen via de opties-pagina van
de extensie.
